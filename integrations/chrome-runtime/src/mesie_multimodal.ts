...
export interface SensePacket { kind: SenseKind; embedding: number[]; confidence: number; salience: number; timestamp: number; source: string; }
export interface FusedSenseState { vector: number[]; coherence: number; dominant: SenseKind[]; phase: 'latent' | 'attentive' | 'burst' | 'integration' | 'consolidation' | 'recovery'; senses: Record<SenseKind, number>; }
...
const cosine = (a: readonly number[], b: readonly number[]): number => {
  let dot = 0, aa = 0, bb = 0; const n = Math.min(a.length, b.length);
...
export class MESIEMultisenseFusion {
  fuse(packets: SensePacket[]): FusedSenseState {
    ...
    for (const packet of packets) {
      ...
      senses[packet.kind] += weight; total += weight;
      for (let i = 0; i < packet.embedding.length; i += 1) vector[i] += packet.embedding[i] * weight;
    }
    const denominator = total || 1; for (let i = 0; i < vector.length; i += 1) vector[i] /= denominator;
    let pairs = 0, coherenceSum = 0;
    for (let i = 0; i < packets.length; i += 1) for (let j = i + 1; j < packets.length; j += 1) { coherenceSum += Math.max(-1, cosine(packets[i].embedding, packets[j].embedding)); pairs += 1; }
