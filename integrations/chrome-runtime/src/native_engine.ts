...
export interface NativeDecodeInput {
  hidden: number[];
  proposal: number[];
  senses?: SensePacket[];
  novelty: number;
...
export interface EfficiencyReceipt {
  schema: 'medina.native.decode.efficiency.v1';
  timestamp: number;
  memorySlots: number;
  memorySelected: number;
  skippedMemorySlots: number;
  residualUpdated: boolean;
  residualGate: number;
  denseAttentionOpsEstimate: number;
  deltaAttentionOpsEstimate: number;
  estimatedAttentionReduction: number;
  phase: FusedSenseState['phase'];
  coherence: number;
  placement?: { nodeId: string; kind: string; score: number };
}

export interface NativeDecodeResult {
  hidden: number[];
  delta: DeltaAttentionResult;
  residual: ResidualDecision;
  senses: FusedSenseState;
...
export class NativeCognitionEngine {
  ...
  step(input: NativeDecodeInput): NativeDecodeResult {
    ...
    const width = Math.max(1, query.length);
    const slots = Math.max(1, this.delta.size());
    const selected = Math.max(1, delta.selected.length);
    const denseOps = slots * width;
    const deltaOps = selected * width;
    const reduction = Math.max(0, 1 - deltaOps / Math.max(1, denseOps));
    const receipt: EfficiencyReceipt = {
      schema: 'medina.native.decode.efficiency.v1',
      timestamp: Date.now(),
      memorySlots: this.delta.size(),
      memorySelected: delta.selected.length,
      skippedMemorySlots: delta.skippedTokens,
      residualUpdated: residual.updated,
      residualGate: residual.gate,
      denseAttentionOpsEstimate: denseOps,
      deltaAttentionOpsEstimate: deltaOps,
      estimatedAttentionReduction: reduction,
      phase: fused.phase,
