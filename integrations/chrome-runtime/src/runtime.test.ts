...
test('adaptive residual bypasses low-value updates', () => {
  const result = new AdaptiveResidualEngine().route([1,1,1], [1.001,1.001,1.001], 0.01, 0.01);
  assert.equal(result.updated, false);
  assert.ok(result.state.bypassRate > 0);
});

test('MESIE fuses multiple senses', () => {
...
test('device fabric prefers capable local WebGPU nodes', () => {
  ...
  assert.equal(fabric.place({ capability:'decode', memoryMB:1024, compute:1, preferLocal:true, maxLatencyMs:100 }).node?.id, 'tab');
});

test('horizon memory retains unresolved goals', () => {
...
test('native engine emits bounded efficiency receipt', () => {
