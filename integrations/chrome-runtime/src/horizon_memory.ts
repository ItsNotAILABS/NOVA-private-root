export interface HorizonEvent { id: string; kind: 'goal' | 'observation' | 'action' | 'result' | 'error' | 'decision' | 'receipt'; text: string; salience: number; unresolved?: boolean; timestamp: number; embedding?: number[]; }
export interface HorizonState { activeGoals: HorizonEvent[]; unresolved: HorizonEvent[]; recent: HorizonEvent[]; compressedSummary: string; eventCount: number; }
...
export class HorizonMemory {
  ...
  resolve(id: string): void { const event = this.events.find((item) => item.id === id); if (event) event.unresolved = false; }
  state(): HorizonState {
    ...
    return { activeGoals, unresolved, recent, compressedSummary: anchors.map((event) => `[${event.kind}:${event.id}] ${event.text}`).join('\n'), eventCount: this.events.length };
  }
  compact(): void {
    const keep = this.state(); const selected = new Map<string,HorizonEvent>();
    for (const event of [...keep.activeGoals, ...keep.unresolved, ...keep.recent]) selected.set(event.id,event);
    this.events.length = 0; this.events.push(...[...selected.values()].sort((a,b) => a.timestamp - b.timestamp));
  }
}
