...
export class DeltaAttentionMemory {
  ...
  snapshot(): DeltaMemoryEntry[] { return this.entries.map((entry) => ({ ...entry, key: [...entry.key], value: [...entry.value] })); }

  restore(entries: DeltaMemoryEntry[]): void {
    this.entries.length = 0;
    for (const entry of entries.slice(0, this.config.capacity)) this.entries.push({ ...entry, key: [...entry.key], value: [...entry.value] });
  }

  attend(query: readonly number[]): DeltaAttentionResult {
    if (!this.entries.length) return { output: new Array(query.length).fill(0), selected: [], attentionMass: 0, memoryWrites: this.writes, skippedTokens: 0 };
    ...
    return { output, selected: ranked.map((item) => item.index), attentionMass: weights.reduce((sum, value) => sum + value, 0), memoryWrites: this.writes, skippedTokens: Math.max(0, this.entries.length - ranked.length) };
  }

  write(key: readonly number[], value: readonly number[]): boolean {
    const bestSimilarity = this.entries.length ? Math.max(...this.entries.map((entry) => cosine(key, entry.key))) : 0;
    ...
    this.entries.push({ key: [...key], value: [...value], utility: 1, novelty, lastUsed: Date.now(), uses: 0 });
    this.writes += 1;
    return true;
  }
}
