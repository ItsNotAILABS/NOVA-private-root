...
export interface DeviceNode { id: string; kind: DeviceClass; memoryMB: number; compute: number; latencyMs: number; battery?: number; thermal?: number; capabilities: string[]; lastSeen: number; }
export interface PlacementRequest { capability: string; memoryMB: number; compute: number; preferLocal?: boolean; maxLatencyMs?: number; }
export interface PlacementDecision { node?: DeviceNode; score: number; reasons: string[]; }
