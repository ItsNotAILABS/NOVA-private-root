...
class NativeArchitectureTests(unittest.TestCase):
    ...
    def test_training_forward_backward(self):
        ...
        self.assertIsNotNone(model.layers[0].attn.k_proj.weight.grad)
        self.assertIsNotNone(model.layers[0].attn.v_proj.weight.grad)
        self.assertGreater(float(model.layers[0].attn.k_proj.weight.grad.abs().sum()), 0.0)

    def test_multisense_native_fusion(self):
        model = NativeTrainingTransformer(self.config, window=4)
        tokens = torch.randint(0, self.config.vocab_size, (2, 8))
        senses = {
            "vision": torch.randn(2, 4, 24),
            "audio": torch.randn(2, 8, 12),
            "telemetry": torch.randn(2, 2, 6),
        }
    ...
    def test_decode_memory_is_bounded(self):
        ...
        state = None
        for _ in range(32):
            token_state = torch.randn(1, self.config.d_model)
            _, state, metrics = attention.decode_step(token_state, state)
        ...
        self.assertLessEqual(metrics["selected_slots"], self.config.top_k * self.config.n_heads)
    ...
    def test_train_decode_state_dict_compatibility(self):
        ...
        missing, unexpected = decode_model.load_state_dict(train_model.state_dict(), strict=False)
    ...
    def test_residual_gates_are_bounded(self):
        model = NativeTrainingTransformer(self.config, window=4)
        tokens = torch.randint(0, self.config.vocab_size, (1, 8))
        gates = model(tokens)["residual_gates"]
