...
class App:
 def __init__(self,library,checkpoint,log):
  from auro_foundry.generation import TextGenerator
  self.lib=Library.load(Path(library));self.h=Hooks(self.lib,log);self.g=TextGenerator(checkpoint,device='cpu')
...
class H(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  if self.path=='/health':self.sendj(200,{'ok':True,'skills':len(self.app.lib.skills),'library_hash':self.app.lib.library_hash})
  elif self.path=='/v1/skills':self.sendj(200,{'skills':[s.__dict__ for s in self.app.lib.skills.values()]})
 ...
 def do_POST(self):
  ...
  if self.path=='/v1/skills/route':d=self.app.lib.route(str(b.get('query','')),4);return self.sendj(200,{'library_hash':d.library_hash,'matches':[m.__dict__ for m in d.matches]})
...
def main():
 p=argparse.ArgumentParser();p.add_argument('--library',required=True);p.add_argument('--checkpoint',required=True);p.add_argument('--host',default='127.0.0.1');p.add_argument('--port',type=int,default=8091);p.add_argument('--usage-log',default='artifacts/skill-usage.jsonl');a=p.parse_args();H.app=App(a.library,a.checkpoint,Path(a.usage_log));s=ThreadingHTTPServer((a.host,a.port),H);print(json.dumps({'ready':True,'url':f'http://{a.host}:{a.port}'}));s.serve_forever()
