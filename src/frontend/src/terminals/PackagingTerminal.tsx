...
import { useCanisterWire, extractPackagingLogs } from './useCanisterWire';
...
export function PackagingTerminal() {
  ...
  const wire = useCanisterWire('PACKAGING', 3000, extractPackagingLogs);
  const packages = getPackagesByDomain('PACKAGING');
  const calls = getCallsByDomain('PACKAGING');
