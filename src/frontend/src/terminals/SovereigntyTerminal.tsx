...
import { useCanisterWire, extractSovereigntyLogs } from './useCanisterWire';
...
export function SovereigntyTerminal() {
  ...
  const wire = useCanisterWire('SOVEREIGNTY', 3000, extractSovereigntyLogs);
  const packages = getPackagesByDomain('SOVEREIGNTY');
  const calls = getCallsByDomain('SOVEREIGNTY');
