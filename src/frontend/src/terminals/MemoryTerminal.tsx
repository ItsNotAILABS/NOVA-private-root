...
import { useCanisterWire, extractMemoryLogs } from './useCanisterWire';
...
export function MemoryTerminal() {
  ...
  const wire = useCanisterWire('MEMORY', 3000, extractMemoryLogs);
  const packages = getPackagesByDomain('MEMORY');
  const calls = getCallsByDomain('MEMORY');
