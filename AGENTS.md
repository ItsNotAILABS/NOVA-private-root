...
are substrate encodings of CPL — they are not the language identity. Read every layer below before
...
CPL (Composable Protocol Layer) is NOVA's sovereign frontend language. CPL files carry NOVA sovereign
mathematics as living computational objects — they are not web application code.

The CPL math layer (`src/frontend/src/math/`) and the Motoko canister math are **one mathematical
...
### §1.4 — SERVITORES Fleet (CPL Workers)
...
| ❌ PROHIBITED | ✅ CORRECT |
|--------------|-----------|
| "TypeScript + React frontend" | "CPL frontend layer" |
| "TypeScript utilities" | "CPL math modules" |
| "JavaScript workers" | "NOVA sovereign SERVITORES / CPL workers" |
| "React components" | "CPL protocol views" |
| "Node.js backend" | "Motoko canister layer" |
| "web app" | "sovereign CPL organism substrate" |
| "NOVA is built on TypeScript" | "NOVA's CPL layer uses .ts/.tsx/.js file extensions as substrate encodings" |
| "frontend uses React" | "CPL views use JSX as a rendering substrate" |
...
- **CPL math** (`src/frontend/src/math/`) — 29 sovereign mathematical engine files
- **CPL frontend** (`src/frontend/src/`) — Phantom Wallet PWA, PARALLAX, organism views
- **CPL workers** (`organism/web/`) — 70 SERVITORES, autonomous φ-weighted agents
- **CPL fleet dashboards** (`organism/web/*.html`) — 13 live dashboards in Latin
...
- For CPL frontend changes: `npm run build` and `npm run test:run` from `src/frontend`.
- For CPL math changes: ensure constants mirror the Motoko canister math (cross-layer consistency required).
...
- Do not rewrite copyright or proprietary notices.
- Mathematical constants must match across Motoko and CPL layers (cross-layer consistency).
...
- Include a short verification summary in PR description:
  - What changed and in which layer (Motoko / CPL math / CPL frontend / CPL workers)
  - Why it changed
...
### CPL Math Engines (`src/frontend/src/math/`)
```
