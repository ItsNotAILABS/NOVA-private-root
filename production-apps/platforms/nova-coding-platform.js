...
const UNIVERSAL_LANG_REGISTRY = {
  ...
  julia: {
    label:    'Julia',
    aliases:  ['jl', '.jl'],
    detect:   [/function\s+\w+\s*\(/, /end\s*$/m, /using\s+\w+/, /::\s*\w+/, /\.\*\s*|\.\/\s*|\.^\s*/],
    primitives: {
      DEFINE:  'function name(param::Type)::ReturnType\n  expression\nend\n# or one-liner:\nname(x) = x^2\n# or anonymous:\nx -> x^2',
      CALL:    'name(arg)  # or  obj.method(arg)  # multiple dispatch based on all arg types',
      BRANCH:  'if cond\n  ...\nelseif cond\n  ...\nelse\n  ...\nend\n# or ternary:  cond ? yes : no',
      REPEAT:  'for item in items\n  ...\nend\n# or  map(fn, items)  # or  [expr for item in items]  # comprehension',
      IMPORT:  'using Module  # or  import Module: name1, name2  # or  include("file.jl")',
      TYPE:    'struct Name\n  field::Type\nend\n# mutable:\nmutable struct Name\n  field::Type\nend\n# abstract type:\nabstract type Parent end',
      ASYNC:   '@async begin\n  result = call()\nend\n# or  @spawn task  # or  Threads.@threads for\n# or Channel/Task for concurrency',
      EMIT:    'return value  # or  println(value)  # or  @show value  # or  throw(ErrorException("msg"))',
    },
    phi_pattern: `const PHI = 1.6180339887498948482\nconst AMOR = PHI^(-2)  # 0.3819...\nconst FEIGENBAUM = 4.6692016091029906718  # δ — chaos constant\n# Vectorised: x .* PHI`,
    comment:  '#',
    block_comment: ['#=', '=#'],
    runtime:  'Julia REPL / Jupyter / Pluto / JuliaHub / HPC / GPU (CUDA.jl)',
  },

  fortran: {
    label:    'Fortran',
    aliases:  ['f90', 'f95', 'f03', 'f08', '.f90', '.f95', '.f03', '.f08', 'fortran'],
    detect:   [/program\s+\w+/i, /subroutine\s+\w+/i, /module\s+\w+/i, /implicit\s+none/i, /intent\s*\(/i],
    primitives: {
      DEFINE:  'function name(param) result(res)\n  real :: param, res\n  res = expression\nend function\n! or subroutine name(param)\n!   ...\n! end subroutine',
      CALL:    'call name(arg)  ! subroutine\nresult = name(arg)  ! function',
      BRANCH:  'if (cond) then\n  ...\nelse if (cond) then\n  ...\nelse\n  ...\nend if\n! or  select case (val)',
      REPEAT:  'do i = 1, n\n  ...\nend do\n! or  do while (cond)\n! or  forall / where for array ops',
      IMPORT:  'use module_name\n! or  use module_name, only: name1, name2\n! or  include "file.f90"',
      TYPE:    'type :: Name\n  real :: field\nend type Name\n! or  type, extends(Parent) :: Child',
      ASYNC:   '! OpenMP: !$omp parallel do\n! MPI: call mpi_send(...)\n! Coarray: image_index(), sync all\n!$omp parallel\n...\n!$omp end parallel',
      EMIT:    'print *, value  ! or  write(*,*) value  ! or  return  ! or  error stop "msg"',
    },
    phi_pattern: `real(8), parameter :: PHI = 1.6180339887498948482d0\nreal(8), parameter :: AMOR = 0.3819660112501051518d0`,
    comment:  '!',
    block_comment: ['!', '!'],
    runtime:  'gfortran / ifort / LLVM Flang / HPC / Supercomputers',
  },

  scala: {
    label:    'Scala',
    aliases:  ['scala', '.scala', '.sc'],
    detect:   [/def\s+\w+\s*[\[(]/, /val\s+\w+\s*:/, /object\s+\w+/, /trait\s+\w+/, /case\s+class/],
    primitives: {
      DEFINE:  'def name(param: Type): ReturnType = {\n  expression\n}\n// or  def name(p: T): R = expr  // single expression',
      CALL:    'name(arg)  // or  obj.name(arg)',
      BRANCH:  'if (cond) expr1 else expr2\n// or  match {\n//   case Pattern => result\n//   case _ => default\n// }',
      REPEAT:  'for (item <- items) yield expr\n// or  items.map(fn)  // or  items.foreach(fn)',
      IMPORT:  'import package.ClassName\n// or  import package._  // wildcard\n// or  import package.{A, B => Alias}',
      TYPE:    'case class Name(field: Type)\n// or  trait Name { def method: Type }\n// or  class Name extends Parent',
      ASYNC:   'import scala.concurrent.Future\nFuture { call() }  // or  for { r <- future } yield r\n// or  Akka actors',
      EMIT:    'expr  // implicit return\n// or  return value  // explicit\n// or  println(value)  // or  throw new Exception("msg")',
    },
    phi_pattern: `val PHI: Double = 1.6180339887498948482\nval AMOR: Double = math.pow(PHI, -2)  // 0.3819...`,
    comment:  '//',
    block_comment: ['/*', '*/'],
    runtime:  'JVM / Scala Native / Scala.js / Spark / Akka',
  },

  elixir: {
    label:    'Elixir',
    aliases:  ['ex', 'exs', '.ex', '.exs', 'elixir'],
    detect:   [/defmodule\s+\w+/, /def\s+\w+\s*[\/(]/, /\|>\s*/, /::\s*\w+/, /do\s*$/m],
    primitives: {
      DEFINE:  'def name(param) do\n  expression\nend\n# or  def name(p), do: expr  # one-liner\n# private:  defp name(p), do: expr',
      CALL:    'name(arg)  # or  Module.name(arg)',
      BRANCH:  'case expr do\n  pattern -> result\n  _ -> default\nend\n# or  if cond do ... else ... end\n# or  cond do',
      REPEAT:  'Enum.map(items, fn item -> expr end)\n# or  for item <- items, do: expr\n# or  Stream for lazy',
      IMPORT:  'import Module  # or  alias Module.Name  # or  use Module  # macro\n# or  require Module  # macros',
      TYPE:    'defmodule Name do\n  defstruct [:field1, :field2]\nend\n# or  @type name :: type\n# typespecs',
      ASYNC:   'Task.async(fn -> call() end)\nTask.await(task)\n# or  spawn(fn -> ... end)  # process\n# or  GenServer for actors',
      EMIT:    'expr  # implicit return\n# or  IO.puts(value)  # or  raise "msg"',
    },
    phi_pattern: `@phi 1.6180339887498948482\n@amor :math.pow(@phi, -2)  # 0.3819...\n# In module: defp phi, do: 1.6180339887498948482`,
    comment:  '#',
    block_comment: ['@doc """', '"""'],
    runtime:  'BEAM VM / Phoenix / LiveView / Nerves (IoT) / Distributed systems',
  },

  lua: {
    label:    'Lua',
    aliases:  ['lua', '.lua'],
    detect:   [/function\s+\w+\s*\(/, /local\s+\w+\s*=/, /then\s*$/, /end\s*$/m, /require\s*\(/],
    primitives: {
      DEFINE:  'function name(param)\n  return expression\nend\n-- or local:\nlocal function name(p) ... end\n-- or anonymous:\nlocal fn = function(x) return x end',
      CALL:    'name(arg)  -- or  obj:method(arg)  -- colon passes self',
      BRANCH:  'if cond then\n  ...\nelseif cond then\n  ...\nelse\n  ...\nend',
      REPEAT:  'for i, item in ipairs(items) do\n  ...\nend\n-- or  for k, v in pairs(tbl)\n-- or  while cond do ... end',
      IMPORT:  "local module = require('module')\n-- or  dofile('file.lua')",
      TYPE:    '-- Lua uses tables:\nlocal Name = {}\nfunction Name:new(p)\n  local o = { field = p }\n  setmetatable(o, { __index = self })\n  return o\nend',
      ASYNC:   '-- Lua is single-threaded. Use coroutines:\nlocal co = coroutine.create(function() ... end)\ncoroutine.resume(co)\n-- or  async libs like copas',
      EMIT:    'return value  -- or  print(value)  -- or  error("msg")',
    },
    phi_pattern: `local PHI = 1.6180339887498948482\nlocal AMOR = PHI ^ -2  -- 0.3819...`,
    comment:  '--',
    block_comment: ['--[[', ']]'],
    runtime:  'Lua interpreter / LuaJIT / Neovim / Game engines (Love2D, Roblox) / Redis',
  },

  zig: {
    label:    'Zig',
    aliases:  ['zig', '.zig'],
    detect:   [/fn\s+\w+\s*\(/, /const\s+\w+\s*=/, /pub\s+fn/, /\|err\|/, /@import/],
    primitives: {
      DEFINE:  'fn name(param: Type) ReturnType {\n  return value;\n}\n// or pub fn for public',
      CALL:    'name(arg)',
      BRANCH:  'if (cond) {\n  ...\n} else if (cond) {\n  ...\n} else {\n  ...\n}\n// or  switch (val) { pattern => expr }',
      REPEAT:  'for (items) |item| {\n  ...\n}\n// or  while (cond) : (update) { }\n// or  inline for',
      IMPORT:  'const std = @import("std");\nconst module = @import("./file.zig");',
      TYPE:    'const Name = struct {\n  field: Type,\n  pub fn method(self: *Name) void { }\n};\n// or  const Name = enum { A, B };',
      ASYNC:   '// Zig uses async/await with suspend points:\nfn asyncFn() void {\n  suspend {}\n}\nvar frame = async asyncFn();\nawait frame;',
      EMIT:    'return value;\n// or  std.debug.print("{}", .{value});\n// or  return error.SomeError;',
    },
    phi_pattern: `const PHI: f64 = 1.6180339887498948482;\nconst AMOR: f64 = 0.3819660112501051518;`,
    comment:  '//',
    block_comment: ['/*', '*/'],
    runtime:  'Native binary / WebAssembly / Embedded / Systems / C interop',
  },

  nim: {
    label:    'Nim',
    aliases:  ['nim', '.nim'],
    detect:   [/proc\s+\w+\s*\(/, /var\s+\w+\s*:/, /import\s+\w+/, /echo\s+/, /result\s*=/],
    primitives: {
      DEFINE:  'proc name(param: Type): ReturnType =\n  result = expression\n# or  func name(p: T): R = expr  # pure',
      CALL:    'name(arg)  # or  obj.name(arg)  # UFCS',
      BRANCH:  'if cond:\n  ...\nelif cond:\n  ...\nelse:\n  ...\n# or  case val\n# of pattern: result',
      REPEAT:  'for item in items:\n  ...\n# or  items.mapIt(it * 2)  # or  while cond:',
      IMPORT:  'import module  # or  from module import name\n# or  include "file.nim"',
      TYPE:    'type\n  Name = object\n    field: Type\n  Name2 = ref object of Parent\n# or  Name = enum A, B, C',
      ASYNC:   'import asyncdispatch\nproc asyncFn(): Future[Type] {.async.} =\n  let r = await call()\n  return r',
      EMIT:    'return value  # or  result = value  # implicit\n# or  echo value  # or  raise newException(E, "msg")',
    },
    phi_pattern: `const PHI = 1.6180339887498948482\nconst AMOR = 0.3819660112501051518`,
    comment:  '#',
    block_comment: ['#[', ']#'],
    runtime:  'Native binary / JavaScript / WebAssembly / Embedded',
  },

  ocaml: {
    label:    'OCaml',
    aliases:  ['ml', 'ocaml', '.ml', '.mli'],
    detect:   [/let\s+\w+\s*=/, /fun\s+\w+\s*->/, /match\s+\w+\s+with/, /module\s+\w+/, /type\s+\w+\s*=/],
    primitives: {
      DEFINE:  'let name param = expression\n(* or with types: *)\nlet name (param : type) : return_type = expression\n(* recursive: *)\nlet rec name param = ...',
      CALL:    'name arg  (* or  Module.name arg *)',
      BRANCH:  'match expr with\n| Pattern1 -> result1\n| Pattern2 -> result2\n| _ -> default\n(* or  if cond then expr1 else expr2 *)',
      REPEAT:  'List.map fn items  (* or *)\nList.iter fn items  (* or *)\nlet rec loop = function [] -> () | x::xs -> ...; loop xs',
      IMPORT:  'open Module  (* or *)\nmodule M = Module  (* alias *)\ninclude Module  (* inline *)',
      TYPE:    'type name = { field : type; ... }\n(* variant: *)\ntype name = A | B of int | C of { x: int; y: int }',
      ASYNC:   '(* Lwt library: *)\nlet%lwt result = call () in\nLwt.return result\n(* or Async library *)',
      EMIT:    'expr  (* implicit return *)\n(* or *) print_endline value\n(* or *) failwith "msg"',
    },
    phi_pattern: `let phi = 1.6180339887498948482\nlet amor = phi ** (-2.)  (* 0.3819... *)`,
    comment:  '(*',
    block_comment: ['(*', '*)'],
    runtime:  'Native binary / js_of_ocaml / Bucklescript/ReScript / MirageOS',
  },

  clojure: {
    label:    'Clojure',
    aliases:  ['clj', 'cljs', 'cljc', '.clj', '.cljs', '.cljc', 'clojure'],
    detect:   [/\(defn\s+\w+/, /\(def\s+\w+/, /\(ns\s+\w+/, /\(fn\s+\[/, /\(let\s+\[/],
    primitives: {
      DEFINE:  '(defn name [param]\n  expression)\n;; or anonymous:\n(fn [x] (* x x))\n;; or short: #(* % %)',
      CALL:    '(name arg)  ; prefix notation\n;; or  (.method obj arg)  ; Java interop',
      BRANCH:  '(cond\n  cond1 result1\n  cond2 result2\n  :else default)\n;; or  (if cond yes no)\n;; or  (case val ...)',
      REPEAT:  '(map fn items)  ; or  (filter pred items)\n;; or  (for [x items] expr)  ; list comprehension\n;; or  (doseq [x items] ...)',
      IMPORT:  "(ns my.namespace\n  (:require [module :as m]))\n;; or  (require '[module :refer [name]])",
      TYPE:    '(defrecord Name [field1 field2])\n;; or  (deftype Name [field] Protocol (method [this] ...))\n;; or  defprotocol',
      ASYNC:   '(future (call))  ; returns future\n@(future ...)  ; deref/await\n;; or  core.async: (go (<! (call)))',
      EMIT:    'expr  ; implicit return (last expression)\n;; or  (println value)\n;; or  (throw (Exception. "msg"))',
    },
    phi_pattern: `(def PHI 1.6180339887498948482)\n(def AMOR (Math/pow PHI -2))  ; 0.3819...`,
    comment:  ';',
    block_comment: ['#_', ''],
    runtime:  'JVM / ClojureScript (JS) / Babashka / GraalVM native',
  },

  dart: {
    label:    'Dart',
    aliases:  ['dart', '.dart'],
    detect:   [/void\s+main\s*\(/, /import\s+'package:/, /class\s+\w+\s*\{/, /final\s+\w+\s*=/, /@override/],
    primitives: {
      DEFINE:  'ReturnType name(Type param) {\n  return value;\n}\n// or arrow:\nReturnType name(Type p) => expression;',
      CALL:    'name(arg)  // or  object.name(arg)',
      BRANCH:  'if (cond) {\n  ...\n} else if (cond) {\n  ...\n} else {\n  ...\n}\n// or  switch (val) { case x: ... }',
      REPEAT:  'for (var item in items) { }\n// or  items.map((x) => x)\n// or  for (var i = 0; i < n; i++)',
      IMPORT:  "import 'package:name/name.dart';\n// or  import 'dart:core';  // standard lib\n// or  import 'file.dart' as alias;",
      TYPE:    'class Name {\n  final Type field;\n  Name(this.field);\n}\n// or  abstract class, mixin, extension',
      ASYNC:   'Future<Type> name() async {\n  final r = await call();\n  return r;\n}\n// or  Stream for reactive',
      EMIT:    'return value;  // or  print(value);  // or  throw Exception("msg");',
    },
    phi_pattern: `const double PHI = 1.6180339887498948482;\nconst double AMOR = 0.3819660112501051518;`,
    comment:  '//',
    block_comment: ['/*', '*/'],
    runtime:  'Dart VM / Flutter (iOS/Android/Web/Desktop) / Server',
  },
};
