...
import { listGates, gateSummary } from './internal-ai/gates.js';
...
import { listUserLanes, getUserLane } from './internal-ai/userLanes.js';
import { capabilityGraph, authorizeCapability } from './internal-ai/capabilityGraph.js';
import { alphaProtocolStatus, routeAlphaRequest } from './internal-ai/alphaProtocolBus.js';
...
export async function route(req, res) {
  ...
  if (pathname === '/api/health') return sendJson(res, 200, { ok: true, app: config.appName, version: config.version, production: true, ai: openAiStatus(), internalAi: internalAiStatus().version, alphaProtocol: alphaProtocolStatus().version });
  ...
  if (pathname === '/api/internal-ai/status') return sendJson(res, 200, { ...internalAiStatus(), alpha: alphaProtocolStatus() });
  if (pathname === '/api/internal-ai/protocol') return sendJson(res, 200, systemProtocolManifest());
  if (pathname === '/api/internal-ai/gates') return sendJson(res, 200, { summary: gateSummary(), gates: listGates(), checklist: gateChecklist() });
  ...
  if (pathname === '/api/internal-ai/user-lanes') return sendJson(res, 200, { lanes: listUserLanes() });
  if (pathname === '/api/internal-ai/user-lane') return sendJson(res, 200, getUserLane(url.searchParams.get('id') || 'founder-operator'));
  if (pathname === '/api/internal-ai/capabilities') return sendJson(res, 200, capabilityGraph());
  if (pathname === '/api/internal-ai/authorize-capability') return sendJson(res, 200, authorizeCapability(url.searchParams.get('organismId') || 'NOVA', url.searchParams.get('capabilityId') || 'runtime_orchestration'));
  if (pathname === '/api/internal-ai/alpha') return sendJson(res, 200, alphaProtocolStatus());
  ...
  if (pathname === '/api/internal-ai/alpha-route' && method === 'POST') {
    const body = await readJson(req);
    return sendJson(res, 200, routeAlphaRequest({ organismId: body.organismId, laneId: body.laneId || body.lane || 'founder-operator', intentText: body.intentText || body.text || '', capabilityId: body.capabilityId }));
  }
